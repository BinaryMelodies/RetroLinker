...
namespace Apple
{
	...
	/**
	 * @brief This is not actually a file format, but an interface to permit generating multiple binary outputs for Apple binaries for their 65xx line.
	 *
	 * Depending on the target disk file system and tools, there are a variety of ways to prepare a binary file. Such files require some
	 * file system specific metadata such as the file type and auxiliary file type that can be stored in several ways:
	 ...
	 */
	class GSOutputDriver : public OutputDriver
	{
	public:
		...
		enum produce_format_t
		{
			...
			PRODUCE_NAPS_SUFFIX = OutputDriver::PRODUCE_NAPS_SUFFIX,
		};
