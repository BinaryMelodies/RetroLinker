...
// TODO: also check for windowed reads
...
void TestReader::testSeekSkip()
{
	...
	CPPUNIT_ASSERT_EQUAL(123, int(static_cast<unsigned char>(buffer[0])));

	reader->Skip(45);
	reader->ReadData(1, buffer);
	CPPUNIT_ASSERT_EQUAL(123 + 1 + 45, int(static_cast<unsigned char>(buffer[0])));
